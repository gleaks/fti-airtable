...
base('Work Orders').select({
  ...
  records.forEach((record) => {
    ...
    if (rs.includes('AC') || rs.includes('DC') || rs.includes('IC')) {
      // makeSystem creates a properly formatted System object, we push it into the array
      systems.push(dict.makeSystem('Testhead', 'A', record.id));
    }
    if (rs.includes('CAL')) systems.push(dict.makeSystem('CAL', 'C', record.id));
    if (rs.includes('DRDSON')) systems.push(dict.makeSystem('DRDSON', 'DRD', record.id));
    if (rs.includes('DXM')) systems.push(dict.makeSystem('DXM', 'D', record.id));
    if (rs.includes('HTS')) systems.push(dict.makeSystem('HTS', 'H', record.id));
    if (rs.includes('IND')) systems.push(dict.makeSystem('Inductor', 'IND', record.id));
    if (rs.includes('MDM')) systems.push(dict.makeSystem('MDM', 'M', record.id));
    if (rs.includes('QC')) systems.push(dict.makeSystem('QG-CAL', 'Q', record.id));
    if (rs.includes('CG')) systems.push(dict.makeSystem('RG-CAL', 'R', record.id));
    if (rs.includes('ULTRA')) systems.push(dict.makeSystem('UHV', 'U', record.id));

    console.log(`Modified ${record.get('Name')} (${record.id})`);
    console.log(systems);

    // We give the entire array of systems to the Airtable API and tell it to create records
